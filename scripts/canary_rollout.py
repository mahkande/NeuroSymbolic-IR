...
def main():
    ...
    should_rollback = (_f(summ.get("error_rate")) > max_error_rate) or (_f(summ.get("p95_latency_ms")) > max_p95) or (pass_rate < min_pass)
    ...
    report = {
        ...
        "canary": canary,
        "decision": {
            "action": "rollback" if should_rollback else "promote",
            "reason": "threshold_violation" if should_rollback else "healthy_canary",
        },
    }
